...
minimal_rows - массив хранящий номера строк матрицы, которым соответствуют узлы с минимальной степенью
rows_count - количество элементов в массиве minimal_rows
*/
void CreateGraphFile(const char* file_name, int* adjacency_matrix, int matrix_size, int* minimal_rows, int rows_count);
...
/*
Если указанный узел имеет минимальную степень, функция возвращяет 1, иначе функция возвращяет 0.
min_rows - массив хранящий номера узлов с минимальной степенью
min_rows_count - длинна массива
node_number - номер узла для которого выполняется проверка
*/
int NodeIsMinimal(const int* min_rows, int min_rows_count, int node_number);
...
int main(void)
{
    ...
    //Минимальная из степеней узлов содержащихся в графе.
    int minimal_degree;
    minimal_degree = GetNodeDegree(adjacency_matrix, nodes_count, 0);//Записываем степень нулевого узла графа.

    for (int node = 1; node < nodes_count; node++)//Перебираем все узлы графа кроме первого. (он был взят на 2-е строки выше)
    {
        //Степель текущего узла.
        int degree = GetNodeDegree(adjacency_matrix, nodes_count, node);
        if (minimal_degree > degree)//Если минимальная степень меньше текущий степени.
        {
            minimal_degree = degree;//Записываем в значение минимальной степени значение степени текущего узла.
        }
    }

    //Массив хранящий списоз узлов с минимальной степенью.
    int* rows_list;

    //Выделяем память под массив.
    rows_list = (int*)malloc(sizeof(int));
    //Количество узлов с минимальной степенью.
    int rows_count = 0;

    for (int node = 0; node < nodes_count; node++)//Прербираем все узлы графа.
    {
        if (GetNodeDegree(adjacency_matrix, nodes_count, node) == minimal_degree)//Если степень текущего числа равна минимальной степени.
        {
            rows_count++;//Увеличиваем (инкременируем) количество элементов в массиве.
            rows_list = (int*)realloc(rows_list, sizeof(int) * rows_count);//Довыделяем память под массив, хранящий списоз узлов с минимальной степенью.
            rows_list[rows_count - 1] = node;//Записывае номер узла, имеющего минимальную степень.
        }
    }

    CreateGraphFile(GRAPH_FILE_MANE, adjacency_matrix, nodes_count, rows_list, rows_count);//Записываем граф в файл, помечая рёбра содержащие хотябы один узел с минимальной степенью.

    free(adjacency_matrix);//Удаляем граф из памяти (очищаем память).
    free(rows_list);//Удаляем массив содержащий список узлов с минимальной степенью.
...
void GetGraphFromConsole(int* adjacency_matrix, int nodes_count)
{
    for (int row = 0; row < nodes_count; row++)//Перебираем все строки в матрице.
    {
        ...
        for (int column = 0; column < nodes_count; column++)//Перебираем все элементы матрицы на текущей строке.
        {
            ...
            adjacency_matrix[GetId(nodes_count, row, column)] = value;//Записываем считанное значени в матрицу, на позицию [row, colunm]
        }
    }
...
int GetNodeDegree(const int* adjacency_matrix, int matrix_size, int node_number)
{
    ...
    for (int column = 0; column < matrix_size; column++)//Переюираем все элементы в матрице смежности на указанной строке. (номер строки соответствует номеру узла)
    {
...
void CreateGraphFile(const char* file_name, int* adjacency_matrix, int matrix_size, int* minimal_rows, int rows_count)
{
    ...
    {
        if (NodeIsMinimal(minimal_rows, rows_count, node))//Если текущий узел требуется раскрасить.
        {
            fprintf(f, "%i [color=red];\n", node);//Записываем узел в файл как раскрашений.
        }
        else//иначе
        {
    ...
    {
        //Флаг хранящий инрмацию имеет ли узел номер row минимальную степень.
        int first_node_is_minimal = NodeIsMinimal(minimal_rows, rows_count, row);
        for (int column = row; column < matrix_size; column++)//Перебираем все элементы на данной строке находящиеся после главной диагонали включительно.
        {
            ...
            if (first_node_is_minimal || NodeIsMinimal(minimal_rows, rows_count, column))//Если один из узлов имеет минимальную степень.
            {
...
int NodeIsMinimal(const int* min_nodes, int min_rows_count, int node_number)
{
    for (int node = 0; node < min_rows_count; node++)//перебираем все элемента массива, хранящего список номеров узлов имеющих минимальную степень
    {
        if (min_nodes[node] == node_number)//Если номер узла в списке совпал с номером узля, для которого проводится проверка, то
        {
    ...
    значит проверяемый узел не имеет минимальную степень,
